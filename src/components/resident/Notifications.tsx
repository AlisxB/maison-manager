...
export const ResidentNotifications: React.FC = () => {
   ...
   const fetchViolations = async () => {
      if (!user) return;
      try {
         const data = await ViolationService.getAll({ resident_id: user.id });
         // Sort by date desc
         setViolations(data.sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime()));
      } catch (error) {
   ...
   if (loading) return <div className="p-8 text-center text-slate-500">Carregando notificações...</div>;
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
               <div className="bg-white rounded-xl shadow-xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                  ...
                  <div className={`p-6 border-b flex justify-between items-start ${selectedViolation.type === 'FINE' ? 'bg-red-50 border-red-100' : 'bg-amber-50 border-amber-100'
                     ...
                     <button onClick={() => setSelectedViolation(null)} className="text-slate-400 hover:text-slate-600 p-1 bg-white/50 rounded-full">
                        <X size={20} />
                     </button>
                  </div>
                     ...
                     {selectedViolation.amount && selectedViolation.amount > 0 && (
                        <div className="flex justify-between items-center py-4 border-t border-b border-slate-100">
                           ...
                           <span className="text-xl font-bold text-slate-900">
                              {FormatCurrency(selectedViolation.amount)}
                           </span>
                        </div>
                     )}
