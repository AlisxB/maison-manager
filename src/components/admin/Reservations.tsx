...
import { MOCK_RESIDENTS } from '../../mock'; // Fallback se precisar
...
export const AdminReservations: React.FC = () => {
    ...
    const [currentMonth, setCurrentMonth] = useState('Dezembro 2025');
    ...
    const handleDateClick = (day: number) => {
        const fullDateStr = `2025-12-${String(day).padStart(2, '0')}`;
    ...
    const handleManualReservationSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const payload = {
                ...
                start_time: `${manualResForm.date}T${manualResForm.startTime}:00`,
                end_time: `${manualResForm.date}T${manualResForm.endTime}:00`
            };

            // Backend currently forces current_user as owner. 
            // In a real generic admin flow, we'd need 'user_id' in payload or assume Admin created it for themselves.
            // For now, let's create it as the Admin himself (simulating) OR if the backend supports override.
            // Looking at reservations.py: user_id=current_user.user_id. So it will be created as the Admin.
    ...
    const getDayContent = (day: number) => {
        ...
        const dayReservations = reservations.filter(r => r.start_time.startsWith(fullDateStr));
        const isBlocked = blockedDates.find(d => d.date === fullDateStr);
                        ...
                        <div className="bg-[#fcfbf9] p-6 rounded-3xl border border-slate-200 shadow-sm flex flex-col h-[700px] animate-in fade-in duration-300">
                            <div className="flex items-center justify-between mb-8">
                                <div className="flex items-center gap-3">
                                    ...
                                    <h3 className="font-black text-slate-700 text-xl">{currentMonth}</h3>
                                </div>
                                <div className="flex gap-2 bg-white p-1 rounded-xl border border-slate-200 shadow-sm">
                                    <button className="p-2 hover:bg-slate-50 rounded-lg text-slate-500 transition-colors"><ChevronLeft size={20} /></button>
                                    <button className="p-2 hover:bg-slate-50 rounded-lg text-slate-500 transition-colors"><ChevronRight size={20} /></button>
                                </div>
                            </div>
                                            ...
                                            <tr key={res.id} className="hover:bg-slate-50/80 transition-colors group">
                                                ...
                                                <td className="px-6 py-4">
                                                    <div className="flex flex-col text-[11px] font-medium">
                                                        ...
                                                        <span className="text-slate-400">{new Date(res.start_time).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}</span>
                                                    </div>
                                                </td>
                                                ...
                                                <td className="px-6 py-4 text-right">
                                                    <div className="flex justify-end gap-2">
                                                        <button className="p-2 text-slate-400 hover:text-[#437476] hover:bg-white rounded-lg transition-all" title="Ver Detalhes">
                                                            <Info size={16} />
                                                        </button>
                                                        <button className="p-2 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-all" title="Cancelar Reserva">
                                                            <Trash2 size={16} />
                                                        </button>
                                                    </div>
                                                </td>
                                    ...
                                    <div key={res.id} className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm hover:border-[#437476] transition-all">
                                        ...
                                        <div className="space-y-2 mb-4 text-slate-600">
                                            ...
                                            <div className="flex items-center gap-2 text-xs">
                                                <Clock size={12} className="text-slate-400" /> {new Date(res.start_time).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}
                                            </div>
                                            <div className="flex items-center gap-2 text-xs">
                                        ...
                                        <div className="flex gap-2">
                                            <button className="flex-1 py-2 bg-green-600 text-white text-[10px] font-black rounded-xl hover:bg-green-700 transition-colors uppercase tracking-wide">Aprovar</button>
                                            <button className="flex-1 py-2 bg-slate-100 text-slate-500 text-[10px] font-black rounded-xl hover:bg-slate-200 transition-colors uppercase tracking-wide">Recusar</button>
                                        </div>
                                    </div>
                                ))
                            ) : (
...
const [activeTab, setActiveTab] = useState<'calendar' | 'list'>('calendar');
const [currentMonth, setCurrentMonth] = useState('Dezembro 2025');
// Estado inicial com algumas datas bloqueadas para teste
const [blockedDates, setBlockedDates] = useState<LocalBlockedDate[]>([
    { date: '2025-12-24', reason: 'Véspera de Natal' },
    { date: '2025-12-25', reason: 'Feriado de Natal' }
]);
...
const handleDateClick = (day: number) => {
    ...
    if (isBlocked) {
        // Se já está bloqueado, o clique abre a opção de DESBLOQUEIO
        setIsUnlockModalOpen(true);
    } else {
        // Se está livre, abre um modal de escolha: Reservar ou Bloquear
        setIsActionChoiceModalOpen(true);
    }
};
...
const handleConfirmUnlock = () => {
    if (selectedDateStr) {
        setBlockedDates(blockedDates.filter(d => d.date !== selectedDateStr));
        setIsUnlockModalOpen(false);
        alert(`Data ${new Date(selectedDateStr).toLocaleDateString('pt-BR')} desbloqueada com sucesso!`);
    }
};

const handleManualReservationSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    alert('Reserva manual criada com sucesso e enviada ao morador!');
    setIsManualReservationModalOpen(false);
};
...
const getDayContent = (day: number) => {
    const fullDateStr = `2025-12-${String(day).padStart(2, '0')}`;
    const dayReservations = MOCK_RESERVATIONS.filter(r => r.date === fullDateStr);
    const isBlocked = blockedDates.find(d => d.date === fullDateStr);
    ...
    <div className="space-y-6">
        ...
        <div className="flex p-1 bg-slate-200/50 rounded-xl w-fit">
            ...
            <button
                ...
                className={`flex items-center gap-2 px-6 py-2.5 rounded-lg text-sm font-bold transition-all ${activeTab === 'list' ? 'bg-white text-[#437476] shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
            >
                <List size={18} /> Reservas Realizadas
            </button>
        </div>
                ...
                {activeTab === 'calendar' ? (
                    ...
                    <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden animate-in fade-in slide-in-from-right-4 duration-300 flex flex-col h-[700px]">
                                ...
                                <tbody className="divide-y divide-slate-100">
                                    {MOCK_RESERVATIONS.map((res) => (
                                        <tr key={res.id} className="hover:bg-slate-50/80 transition-colors group">
                                            ...
                                            <td className="px-6 py-4">
                                                <span className="text-xs font-bold text-slate-600">{res.area}</span>
                                            </td>
                                            <td className="px-6 py-4">
                                                <div className="flex flex-col text-[11px] font-medium">
                                                    <span className="text-slate-700 font-bold">{new Date(res.date).toLocaleDateString('pt-BR')}</span>
                                            ...
                                            <td className="px-6 py-4 text-center">
                                                <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest border ${res.status === 'Confirmed' ? 'bg-green-50 text-green-700 border-green-100' :
                                                    ...
                                                    {res.status === 'Confirmed' ? 'Confirmada' :
                                                        res.status === 'Pending' ? 'Pendente' : 'Rejeitada'}
                                                </span>
                                            </td>
                                            <td className="px-6 py-4 text-right">
                                                <div className="flex justify-end gap-2">
                        ...
                        <div className="p-4 bg-slate-50/50 border-t border-slate-100 text-center">
                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Exibindo {MOCK_RESERVATIONS.length} reservas registradas</p>
                        </div>
                    </div>
                )}
            </div>

            {/* Sidebar de Solicitações (Sempre visível para alertar o admin) */}
            <div className="space-y-6">
                <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden flex flex-col h-[700px]">
                    <div className="p-5 border-b border-slate-100 bg-slate-50/50">
                        <h3 className="font-black text-slate-700 flex items-center gap-2">
                    ...
                    <div className="overflow-y-auto custom-scrollbar p-5 space-y-4 flex-1">
                        {MOCK_RESERVATIONS.filter(r => r.status === 'Pending').length > 0 ? (
                            MOCK_RESERVATIONS.filter(r => r.status === 'Pending').map(res => (
                                <div key={res.id} className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm hover:border-[#437476] transition-all">
                                    <div className="flex justify-between items-start mb-3">
                                        <span className="font-black text-[#437476] text-[10px] uppercase tracking-wider">{res.area}</span>
                                        <span className="bg-amber-100 text-amber-700 px-2 py-0.5 rounded text-[9px] font-black uppercase">Solicitado</span>
                                    </div>
                                    <div className="space-y-2 mb-4 text-slate-600">
                                        <div className="flex items-center gap-2 text-xs font-bold">
                                            <Calendar size={12} className="text-slate-400" /> {new Date(res.date).toLocaleDateString('pt-BR')}
                                        </div>
                                        <div className="flex items-center gap-2 text-xs">
                                            <Clock size={12} className="text-slate-400" /> {res.time}
                                        </div>
                                        <div className="flex items-center gap-2 text-xs">
                                            <User size={12} className="text-slate-400" /> {res.residentName} • {res.unit}
                                        </div>
                                    </div>
                                    <div className="flex gap-2">
                                        <button className="flex-1 py-2 bg-green-600 text-white text-[10px] font-black rounded-xl hover:bg-green-700 transition-colors uppercase tracking-wide">Aprovar</button>
        ...
        {/* MODAL DE ESCOLHA DE AÇÃO (Abre ao clicar em data livre) */}
        {isActionChoiceModalOpen && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
                <div className="bg-[#fcfbf9] rounded-[2.5rem] shadow-2xl w-full max-w-sm overflow-hidden animate-in fade-in zoom-in-95 duration-200 relative border border-slate-200">
                    <button onClick={() => setIsActionChoiceModalOpen(false)} className="absolute right-6 top-6 text-slate-400 hover:text-slate-600 z-10 p-2"><X size={20} /></button>
                    <div className="px-8 py-10 text-center">
                        <div className="w-16 h-16 bg-[#437476]/10 text-[#437476] rounded-2xl flex items-center justify-center mx-auto mb-6 shadow-lg shadow-[#437476]/5"><Calendar size={32} /></div>
                        <h3 className="text-xl font-black text-slate-700 mb-2">
                            {selectedDateStr ? new Date(selectedDateStr + 'T12:00:00').toLocaleDateString('pt-BR') : 'Data Selecionada'}
                        </h3>
                        <p className="text-sm text-slate-500 font-medium mb-8">O que deseja realizar nesta data?</p>
                        <div className="flex flex-col gap-3">
                            <button onClick={openManualReservation} className="w-full py-4 bg-[#437476] text-white font-black rounded-2xl hover:bg-[#365e5f] transition-all flex items-center justify-center gap-2 shadow-lg shadow-[#437476]/20 uppercase tracking-widest text-[10px]">
                ...
                <div className="bg-[#fcfbf9] rounded-[2.5rem] shadow-2xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in-95 duration-200 relative border border-slate-200">
                    ...
                    <form onSubmit={handleManualReservationSubmit} className="p-8 space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <div className="relative group">
                                    <select
                                        className="w-full px-5 py-3.5 bg-white border border-slate-200 rounded-2xl text-sm font-bold text-slate-700 outline-none focus:ring-4 focus:ring-[#437476]/5 focus:border-[#437476] appearance-none shadow-sm transition-all"
                                        value={manualResForm.area}
                                        onChange={e => setManualResForm({ ...manualResForm, area: e.target.value })}
                                    >
                                        <option>Salão de Festas</option>
                                        <option>Churrasqueira</option>
                                        <option>Quadra de Tênis</option>
                                        <option>Espaço Gourmet</option>
                                    </select>
                                    <ChevronDown size={14} className="absolute right-5 top-4.5 text-slate-400 pointer-events-none" />
                                </div>
                            </div>
                        ...
                        <div className="bg-blue-50 border border-blue-100 p-5 rounded-2xl flex gap-3 items-start shadow-sm shadow-blue-50">
                            <Info size={18} className="text-blue-500 mt-0.5" />
                            <p className="text-[10px] text-blue-700 leading-relaxed font-black uppercase tracking-tight">
                                Esta reserva será registrada como <strong className="text-blue-900 underline">Confirmada</strong> imediatamente. O morador será notificado via painel e WhatsApp.
                            </p>
                        </div>
