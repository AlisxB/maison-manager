...
import { UserCheck, CheckCircle, UserPlus, Eye, X, Calendar, Phone, Mail, FileText, User } from 'lucide-react';
import { MOCK_REQUESTS } from '../../mock';
...
export const AdminRequests: React.FC = () => {
  ...
  const handleApprove = () => {
    alert(`Aprovando ${selectedRequest?.name}`);
    setSelectedRequest(null);
  };

  const handleReject = () => {
    alert(`Rejeitando ${selectedRequest?.name}`);
    setSelectedRequest(null);
  };
            ...
            <div key={req.id} className="bg-white p-6 rounded-lg border border-slate-200 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-6">
                <div className="flex items-center gap-4 w-full sm:w-auto">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-slate-500">{req.type} • Unidade {req.unit}</p>
                        <p className="text-xs text-slate-400 mt-1">{req.email} • Solicitado {req.date}</p>
                    </div>
                </div>
                <div className="flex gap-3 w-full sm:w-auto">
                    ...
                    <button className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-4 py-2 bg-[#437476] text-white rounded-lg hover:bg-[#365e5f] text-sm font-medium transition-colors">
                        <CheckCircle size={16} /> Aprovar
                    </button>
                </div>
            </div>
        ))}
        {MOCK_REQUESTS.length === 0 && <div className="text-center text-slate-500 py-10">Nenhuma solicitação pendente.</div>}
      </div>
                 ...
                 <div className="flex items-center gap-3 mb-6">
                    ...
                    <div>
                        ...
                        <div className="flex gap-2 mt-1">
                            <span className="px-2 py-0.5 bg-blue-100 text-blue-700 rounded text-xs font-bold uppercase">{selectedRequest.type}</span>
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="flex items-start gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-sm font-bold text-slate-800 text-lg">{selectedRequest.unit}</p>
                            </div>
                        </div>
                        ...
                        <div className="flex items-start gap-3">
                            ...
                            <div>
                                <p className="text-xs text-slate-500">Previsão de Mudança</p>
                                <p className="text-sm font-medium text-slate-800">{selectedRequest.moveInDate ? new Date(selectedRequest.moveInDate).toLocaleDateString('pt-BR') : 'Não informada'}</p>
                            </div>
                        </div>
                    </div>
                 </div>
                 ...
                 {selectedRequest.observations && (
                    <div className="mt-8">
                        ...
                        <div className="bg-amber-50 border border-amber-100 rounded-lg p-4 flex gap-3">
                            ...
                            <p className="text-sm text-slate-700 italic">"{selectedRequest.observations}"</p>
                        </div>
                    </div>
                 )}
              </div>
