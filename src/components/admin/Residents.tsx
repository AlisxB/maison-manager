            ...
            {MOCK_RESIDENTS.map((res) => (
              <tr key={res.id} className="hover:bg-slate-50">
                ...
                <td className="px-6 py-4 text-center">
                    <span className={`px-2 py-1 rounded-full text-xs font-bold ${res.status === 'Ativo' ? 'bg-green-100 text-green-700' : res.status === 'Pendente' ? 'bg-yellow-100 text-yellow-700' : 'bg-slate-100 text-slate-600'}`}>
                        {res.status}
                    </span>
                </td>
           ...
           <div className="bg-[#fcfbf9] rounded-lg shadow-xl w-full max-w-4xl overflow-hidden animate-in fade-in zoom-in-95 duration-200 relative">
              ...
              <div className="px-8 py-6 max-h-[75vh] overflow-y-auto custom-scrollbar bg-[#fcfbf9]">
                 <div className="space-y-5">
                    ...
                    <button className="w-full py-4 bg-[#437476] text-white font-medium rounded-lg hover:bg-[#365e5f] shadow-sm transition-colors text-base" onClick={() => setIsAddResidentModalOpen(false)}>
                       Salvar Morador
                    </button>
                 </div>
              </div>
